$RemoteComputers = Get-Content -Path "$PSScriptRoot\Computers.txt"
$NewLines = "---------------------------------"
...
$Newlines, $Newlines, $RemoteComputer, $Processor, $Drive, $RAM | Out-File -append -FilePath "$PSScriptRoot\Results.txt"
}
  else{$Newlines, $Newlines, $RemoteComputer, $Offline |Out-File -append -FilePath "$PSScriptRoot\Results.txt" }
}
Write-Host "Done"
notepad "$PSScriptRoot\Results.txt"