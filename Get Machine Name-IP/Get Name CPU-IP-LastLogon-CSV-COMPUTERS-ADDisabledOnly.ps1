...
Import-Module activedirectory
        ...
        $Computers = ( Get-ADComputer -filter ('Name -like "*' + $RemoteComputer + '*"') -Properties OperatingSystem | Where-Object {$_.Distinguishedname -match 'ou=disabled'}).name | sort
...
$Computer
    ...
    $IP1 = $IP -replace ".$"
    $IP = $IP1.substring(14)
    New-Object psobject -Property @{Name=$Computer;CPU=$CPU;IP=$IP;LastLogon=""} | export-csv -append "$PSScriptRoot\$RemoteComputer Disabled.csv"}
    ...
    New-Object psobject -Property @{Name=$Computer;CPU="";IP="";LastLogon=$Logon} | export-csv -append "$PSScriptRoot\$RemoteComputer Disabled.csv"}
}
catch {
